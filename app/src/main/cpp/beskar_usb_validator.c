...
#define LOG_TAG "BeskarUSBValidator"
...
Java_com_thisistheway_grogu_UsbDeviceChecker_beskarUsbDescriptor(JNIEnv *env, jobject instance, jint fileDescriptor)
{
    ...
    LOGD("BESKAR_USB: PASSED LIBUSB_SET NO DISCOVERY");
    ...
    LOGD("BESKAR_USB: PASSED LIBUSB_INIT");
    ...
    LOGD("BESKAR_USB: PASSED WRAP SYS DEVICE");

    // Detach the kernel driver if necessary
    if (libusb_kernel_driver_active(devh, 0) == 1) {
        LOGD("BESKAR_USB: KERNEL DRIVER ACTIVE - TESTING DE-COUPLING");
        r = libusb_detach_kernel_driver(devh, 0);
        if (r != LIBUSB_SUCCESS) {
            LOGD("Failed to detach kernel driver: %d\n", r);
            libusb_close(devh);
            libusb_exit(ctx);
            return;
        }
    }
    LOGD("BESKAR_USB: KERNEL DRIVER RE-ATTACHED");
    // Claim the first interface (adjust based on your device)
    r = libusb_claim_interface(devh, 0);
    if (r < 0)
    {
        LOGD("libusb_claim_interface failed: %d\n", r);
        libusb_close(devh);
        libusb_exit(ctx);
        return;
    }
    LOGD("BESKAR_USB: INTERFACE RE-CLAIMED FROM THIS PROCESS - SUCCESS");


    // Release the interface
    r = libusb_release_interface(devh, 0);
    if (r != LIBUSB_SUCCESS) {
        LOGD("libusb_release_interface failed: %d\n", r);
    }

    LOGD("BESKAR_USB: INTERFACE RELEASE - SUCCESS");

    // Reattach the kernel driver
    r = libusb_attach_kernel_driver(devh, 0);
    if (r != LIBUSB_SUCCESS) {
        LOGD("libusb_attach_kernel_driver failed: %d\n", r);
    }

    LOGD("BESKAR_USB: SUCCESS - ALL TESTS PASSED IN UsbDeviceChecker");
    // Print device details without closing the handle
    LOGD("BESKAR_USB: DEVICE INFORMATION: ");
    print_device(libusb_get_device(devh), devh);
    // Close the device handle and exit
    libusb_close(devh);
    libusb_exit(ctx);
}
